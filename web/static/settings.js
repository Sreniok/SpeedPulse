...
function populateSettingsForm(payload) {
  ...
  const providerSelect = byId("settings-contract-provider-select");
  const providerCustomInput = byId("settings-contract-provider-custom");
  if (providerSelect) {
    providerSelect.value =
      selectedProvider &&
      selectedProvider.toLowerCase() !== detectedProvider.trim().toLowerCase()
        ? "__custom__"
        : "";
  }
  if (providerCustomInput) {
    providerCustomInput.value =
      providerSelect?.value === "__custom__" ? selectedProvider : "";
  }
...
function syncContractProviderPicker() {
  ...
  const customInput = byId("settings-contract-provider-custom");
  if (!detectedInput || !contractProviderSelect || !customGroup || !customInput) return;

  const detectedProvider = detectedInput.value.trim();
  const autoOption = contractProviderSelect.querySelector('option[value=""]');
  if (autoOption) {
    autoOption.textContent = detectedProvider
      ? `Auto detect (${detectedProvider})`
      : "Auto detect";
  }
  ...
  if (contractProviderSelect.value !== "__custom__") {
    customInput.value = "";
  }
}
